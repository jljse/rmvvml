...
namespace Rmvvml
{
    ...
    /// 
    /// 使用方法
    /// 1. ViewにAttachedPropertyAction型の添付プロパティを定義します
    /// 2. プロパティ変更イベントの中で、AttachedPropertyActionHandler.Injectで実行したい処理を登録します
    /// 3. ViewModelにAttachedPropertyAction型のプロパティを定義します
    /// 4. XAMLで1の添付プロパティと3のプロパティをバインドします
    /// 5. ViewModelからInvokeメソッドを呼ぶことで、2の処理が実行されるようになります
    /// </summary>
    /// <typeparam name="TParam">通知パラメータの型</typeparam>
    public class AttachedPropertyAction<TParam>
    {
        // Viewで実行する処理の集合
        // 役割は単なるdelegateと同じですが、以下の点が違います
        // - Viewの強参照を持たない
        // - lambda式を登録してもremoveできる方法を提供している
        List<IAttachedPropertyActionHandler<TParam>> Handlers { get; } = new List<IAttachedPropertyActionHandler<TParam>>();

        /// <summary>
        /// View側からハンドラを登録する
        /// </summary>
        /// <param name="handler"></param>
        public void Add(IAttachedPropertyActionHandler<TParam> handler)
        {
            GC();
            // System.Diagnostics.Debug.WriteLine("AttachedPropertyAction.Add add handler");
        ...
        public void Remove(string attachedPropertyName, object obj)
        {
            for (int i = 0; i < Handlers.Count; ++i)
            {
                ...
                {
                    // System.Diagnostics.Debug.WriteLine("AttachedPropertyAction.Remove remove handler");
                    Handlers.RemoveAt(i);
                }
            }
        }
        ...
        void GC()
        {
            for(int i = Handlers.Count - 1; i >= 0; --i)
            {
                ...
                if(!handler.IsAlive())
                {
                    // System.Diagnostics.Debug.WriteLine("AttachedPropertyAction.GC remove handler");
                    Handlers.RemoveAt(i);
                }
            }
        }
    ...
    /// 通知引数が不要な場合用
    /// </summary>
    public class AttachedPropertyAction : AttachedPropertyAction<object>
    {
        /// <summary>
        /// Viewへの通知を実行します
        /// </summary>
    ...
    public interface IAttachedPropertyActionHandler<TParam>
    {
        /// <summary>
    ...
    public class AttachedPropertyActionHandler<TControl, TParam> : IAttachedPropertyActionHandler<TParam>
        where TControl : FrameworkElement
    {
        AttachedPropertyActionHandler(string attachedPropertyName, TControl control, Action<TControl, TParam> action)
        {
        ...
        public static void Inject(DependencyProperty attachedProperty, DependencyObject d, DependencyPropertyChangedEventArgs e, Action<TControl, TParam> action)
        {
            ...
            var oldVal = e.OldValue as AttachedPropertyAction<TParam>;
            if (oldVal != null)
            {
                // 古いハンドラを削除
                oldVal.Remove(attachedProperty.Name, control);
            }
            var newVal = e.NewValue as AttachedPropertyAction<TParam>;
            if (newVal != null)
            {
                // 新しいハンドラを登録
                var handler = new AttachedPropertyActionHandler<TControl, TParam>(attachedProperty.Name, control, action);
                newVal.Add(handler);
            }
        }
    }
